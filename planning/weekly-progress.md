...
- [x] RC/RL first-order step response and time constants (tau = RC, tau = L/R)
...
- time-constant reading in both directions: ODE -> tau = RC; measured i(t) = 0.02 e^{-500t} A -> tau = 2 ms
- RLC ladder R = 16/20/40 ohm -> zeta = 0.8/1/2 classified by hand and confirmed by simulation (numerical error about 10^{-9} V)

### Review queue

1. Second-derivative notation d^2vC/dt^2 (flagged twice on Day 3: diagnostic Q4 and exit test ET4).
